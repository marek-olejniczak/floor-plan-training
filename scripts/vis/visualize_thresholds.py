#!/usr/bin/env python3
"""
visualize_thresholds.py — 10 images × 3 area thresholds (P85/P90/P95).
Compare filtering results side by side on the same images.
In-memory filtering, no disk writes.
"""
...
OUTPUT = Path("/mnt/d/rzuty/trening") / "threshold_review.png"
...
def parse_label_line(line):
    ...
    # YOLO (cls cx cy w h conf fs) — 7 pol, lub bez fs — 6 pol
    if len(coords) >= 6:
        conf = coords[4]
...
def filter_windows(lines, max_window_area, wall_bboxes):
    results = []
    for line in lines:
...
def create_montage():
    # Zbierz obrazy z area > 0 (przy P90 jako referencyjnym)
    candidates = []
    ...
    print(f"  Wybrano {len(selected)} obrazow z area>P85")
    ...
    # Naglowki kolumn
    for ci, (label, thresh) in enumerate(THRESHOLDS):
        hx = ci * CELL_W + CELL_W // 2
        text = f"{label} = {thresh:.4f}"
    ...
    draw.text((20, 42),
        f"overlap threshold: < {MIN_WALL_OVERLAP}  |  area thresholds: P85=0.0083  P90=0.0869  P95=0.2376",
        fill=(180, 180, 180), font=font_sm)
    draw.text((20, 64),
        "Each row = same image  |  Each column = different area threshold",
    ...
    for ci, (label, thresh) in enumerate(THRESHOLDS):
        print(f"    Kolumna {ci}: {label} = {thresh:.4f}")
