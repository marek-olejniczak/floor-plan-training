...
def create_montage():
    ...
    for split in ("train", "valid", "test"):
        ...
        for img_path in sorted(img_dir.iterdir()):
            ...
            # Wybierz obrazy gdzie przy overlap=0.3 jest ≥1 kept I ≥1 filtered
            n_kept_test = n_filtered_test = 0
            for line in lines:
                parsed = parse_label_line(line)
                if parsed and parsed[0] == 2:
                    ...
                    if w_b * h_b > MAX_WINDOW_AREA:
                        ...
                        cy = (min(ys) + max(ys)) / 2
                        win_bbox = (cx - w_b/2, cy - h_b/2, cx + w_b/2, cy + h_b/2)
                        if bbox_overlap_ratio(win_bbox, wall_bboxes) < 0.3:
                            n_filtered_test += 1
                        else:
                            n_kept_test += 1
            if n_kept_test > 0 and n_filtered_test > 0:
    ...
    print(f"  Wybrano {len(selected)} obrazow (kept>0 i filtered>0 przy overlap=0.3)")
